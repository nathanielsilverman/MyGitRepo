...
public class Message {
     ...
     *
     * @param networkPayload byte[] of the network payload read off of an input stream
     */
    public Message(byte[] networkPayload) {
        ...
        this.errorOccurred = buffer.get() == (byte) 1;
    ...
    public String toString(){
        if(this.stringRepresentation == null){
            ...
            if(getMessageType() == MessageType.ELECTION){
                ...
                builder.append("\n\tSender server state: ");
                builder.append(ZooKeeperPeerServer.ServerState.getServerState(stateChar));
                builder.append("\n\tSender server ID: ");
                ...
                builder.append("\nGossip Message Sent From Server-" + String.format("%03d", senderID));
                long targetID = buffer.getLong();
                builder.append("\nReceived By Server-" + String.format("%03d", targetID));
                int numberEntries = buffer.getInt();
                builder.append("\n -----------------");
                builder.append("\n| ID: | HeartBeat |\n");
                for (int i = 0; i < numberEntries && buffer.remaining() >= 16; i++) {
                    long id = buffer.getLong();
                    long hbCount = buffer.getLong();
                    builder.append("| ");
                    builder.append(String.format("%03d", id));
                    builder.append(" | ");
                    builder.append(String.format("%09d", hbCount));
                    builder.append(" |\n");
                }
                builder.append(" -----------------\n");
            }
